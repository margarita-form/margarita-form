...
class RefManager<CONTROL extends MFC> extends BaseManager {
  ...
  _defineControlsToNode(node: MargaritaFormBaseElement<CONTROL>) {
    if (!node) return;
  ...
  _connectNodeAndControl(node: MargaritaFormBaseElement<CONTROL>) {
    if (!node) return;
  ...
  _removeControlFromNode(node: MargaritaFormBaseElement<CONTROL>) {
    if (!node) return;
    this._defineControlsToNode(node);
  ...
  _deleteNodeRef(node: MargaritaFormBaseElement<CONTROL>) {
    if (!node) return;
